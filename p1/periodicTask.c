...
struct thread_args {
    char p_name[40];
    unsigned long periodicity_s;
    unsigned long periodicity_ns;
};
...
void * Thread_1_code(void *arg)
{
  ...
  //
	tp.tv_nsec = ((struct thread_args*)arg)->periodicity_ns;
...
int main(int argc, char *argv[])
{
  ...
  pthread_attr_setschedparam(&attr, &parm);
	err=pthread_create(&threadid, &attr, Thread_1_code, th_ag );
